...
const Swaps = () => {
  ...
  useEffect(() => {
    const fetchSwaps = async () => {
      try {
        ...
        const res = await axios.get('https://astonishing-enchantment-production.up.railway.app/api/swaps/my-swaps', {
  ...
  const handleUpdateStatus = async (swapId, status) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://astonishing-enchantment-production.up.railway.app/api/swaps/${swapId}`, { status }, {
        headers: { 'x-auth-token': token }
      });
